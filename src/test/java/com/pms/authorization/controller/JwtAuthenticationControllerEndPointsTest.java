...
class JwtAuthenticationControllerEndPointsTest {
	...
	void testBadRequestGenerateToken() throws Exception {
		this.mockMvc.perform(post("/api/final/authenticate")).andExpect(status().isBadRequest());
	}
	...
	void testAuthorizedGenerateToken() throws Exception {
		...
		this.mockMvc.perform(post("/api/final/authenticate").contentType(MediaType.APPLICATION_JSON).content(jsonString))
	...
	void testRandomUrl() throws Exception {
		this.mockMvc.perform(get("/api/final/other/url")).andExpect(status().isNotFound());
	}
	...
	void textExistingUserAuthenticate() throws Exception {
		...
		ObjectMapper mapper = new ObjectMapper();
		mockMvc.perform(MockMvcRequestBuilders.post("/api/final/authenticate").contentType(MediaType.APPLICATION_JSON)
	...
	void textExistingUserAuthorize() throws Exception {
		...
		when(userDetailsService.loadUserByUsername("admin")).thenReturn(details);
		when(jwtTokenUtil.getUsernameFromToken("token")).thenReturn("admin");
		mockMvc.perform(MockMvcRequestBuilders.post("/api/final/authorize").header("Authorization", "Bearer token"))
	...
	void textNullTokenAuthorize() throws Exception {
		...
		when(userDetailsService.loadUserByUsername("admin")).thenReturn(details);
		when(jwtTokenUtil.getUsernameFromToken("token")).thenReturn("admin");
		mockMvc.perform(MockMvcRequestBuilders.post("/api/final/authorize").header("Authorization", "")
				.contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk());

	}
	@Test
    void heatlthCheck() throws Exception {
        this.mockMvc.perform(get("/api/final/health-check")).andExpect(status().isOk());
    }
